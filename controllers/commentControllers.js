...
// DELETE -> `/comments/delete/<sameFruitId>/<someCommentId>`
// make sure only the author of the comment can delete the comment
router.delete('/delete/:fruitId/:commId', (req, res) => {
  // isolate the ids and save to variables so we dont have to keep typing req.params
  // const fruitId = req.params.fruitId
  ...
  Fruit.findById(fruitId)
  .then(fruit => {
    // get the comment, well use the built in subdoc method called .id()
    const theComment = fruit.comments.id(commId)
    console.log('this is the comment to be deleted: \n', theComment)
    // then we want to make sure the user is logged in and they are the author of the comment
    if (req.session.loggedIn) {
      // if they are the author of the comment, allow them to delete
